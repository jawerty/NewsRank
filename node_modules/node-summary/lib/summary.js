...
function splitContentToSentences(content, callback) {
	...
	callback(_sbd2.default.sentences(content, { newline_boundaries: true }) || []);
}
...
function getSortedSentences(paragraph, sentences_dict, n, callback) {
	splitContentToSentences(paragraph, function (sentences) {
		...
		var sentence_scores = [],
		    strip_s;
		_lodash2.default.each(sentences, function (s, i) {
		...
		sentence_scores = _lodash2.default.sortBy(sentence_scores, function (sentence_score) {
		...
		sentence_scores = _lodash2.default.sortBy(sentence_scores, function (sentence) {
			return sentence.order;
		});

		callback(_lodash2.default.map(sentence_scores, 'sentence')); // callback with sorted_sentences. _.map is former .pluck
	});
}

function getSentencesRanks(content, callback, sentences_dict) {
	if (sentences_dict !== undefined) {
		// return cached sentences_dict if available
		callback(sentences_dict);
		return;
	} else sentences_dict = {};
	splitContentToSentences(content, function (sentences) {
		var n = sentences.length,
		    zeroNRange = _lodash2.default.range(0, n),
		    nRange = _lodash2.default.range(n);

		// This is ugly, I know.
		var values = [],
		    _val = [];
		_lodash2.default.each(nRange, function (x) {
			_val = [];
			_lodash2.default.each(nRange, function (y) {
				_val.push(0);
			});
			values.push(_val);
		});
		// Assign each score to each sentence
		_lodash2.default.each(zeroNRange, function (i) {
			_lodash2.default.each(zeroNRange, function (j) {
		...
		var score = 0;
		_lodash2.default.some(zeroNRange, function (i) {
			score = 0;
			_lodash2.default.some(zeroNRange, function (j) {
...
exports.summarizeFromUrl = function (url, callback) {
	...
	if (isValidUrl(url)) {
		_request2.default.get(url, function (error, response, body) {
			...
			var text = convertHTMLToText(body);
			var content = onlyGetSentences(text);
...
exports.summarize = function (title, content, callback, sentences_dict) {
	...
	getSentencesRanks(content, function (dict) {
		splitContentToParagraphs(content, function (paragraphs) {
			...
			_lodash2.default.each(paragraphs, function (p) {
